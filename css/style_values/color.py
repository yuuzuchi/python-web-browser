...
class Color:
    ...
    def __init__(self, r, g, b, a=255):
        ...
        self.color = (r << 24) | (g << 16) | (b << 8) | a

    @classmethod
    def from_hash_str(cls, hash_str: str) -> "Color":
        if hash_str.startswith("#"):
            hash_str = hash_str[1:]

        if len(hash_str) == 3:
            r = int(hash_str[0] * 2, 16)
            g = int(hash_str[1] * 2, 16)
            b = int(hash_str[2] * 2, 16)
            a = 255
        elif len(hash_str) == 6:
            r = int(hash_str[:2], 16)
