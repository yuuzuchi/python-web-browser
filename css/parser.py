...
# Container classes, only used while parsing
# For computing styles, CSSStylesheet, StyleRule, and StyleDeclaration are used instead
# as they contain parsed StyleValues instead of token lists
...
class Rule:
    ...
    def __str__(self):
        return f"{"".join(map(str, self.prelude))} {self.block}"
...
class AtRule(Rule):
    ...
    def __str__(self):
        prelude = "".join(map(str, self.prelude))
        res = f"@{self.name} {prelude}"
        if self.block:
            res += f" {self.block}"
        else:
            res += ";"
...
class Declaration:
    ...
    def __str__(self):
        ...
        if isinstance(vals, list):
            vals = "".join(map(str, self.val))
        return f"{self.name}: {vals}{" !important" if self.important else ""};"
...
class CSSSyntaxParser:
    ...
    def parse_css_stylesheet(self, inp, parse_context: ParseContext) -> CSSStylesheet:
        self.init_state(inp)
        out = []
        ...
        for i, rule in enumerate(stylesheet.val):
            ...
            declarations = declaration_parser.parse_style_block_contents(rule.block.val)
            ...
            out.append(
                StyleRule(
                    selector_list=stylesheet.val[i].prelude,
                    declarations=temp,
                    source_order=parse_context.source_order,
                    origin=parse_context.origin,
                )
            )
    ...
    def parse_css_style_attribute(
        ...
        self.init_state(inp)
        out = []
        declarations = self.parse_declaration_list(self.input)
    ...
    def parse_comma_separated_list(self, inp) -> list[Component | Token]:
        ...
        # parse each component value list according to parse_function
    ...
    def parse_stylesheet(self, inp, location: str = "") -> Stylesheet:
    ...
    def consume_declaration(self) -> Declaration:
        ...
        while declaration.val and declaration.val[-1].type == Tok.WHITESPACE:
            declaration.val.pop()
...
class SelectorParser:
    ...
    def parse_complex_selector_list(self) -> list[ComplexSelector]:
        ...
        while True:
            ...
            if not self.stream.accept(type=Tok.COMMA):
                return out
            self.stream.consume_whitespace()
    ...
    def parse_compound_selector(self) -> CompoundSelector:
        ...
        # [ <pseudo-element-selector> <pseudo-class-selector>* ]*
        while self.stream.peek().type == Tok.COLON:
            if self.stream.peek(2).type == Tok.COLON:
                pseudo_elements.append(self.parse_pseudo_element_selector())

            if pseudo_elements:
                # consume any amount of pseudo-class-selectors
                while (
                    self.stream.peek(2).type == Tok.IDENT
                    or self.stream.peek(2).type == Tok.FUNCTION
                ):
    ...
    def parse_type_selector(self) -> TypeSelector | UniversalSelector:
        if self.is_wq_name():
            ...
            return TypeSelector(name.casefold(), namespace)
        else:
    ...
    def parse_wq_name(self) -> tuple[str, str]:
        ...
        if ident.type != Tok.IDENT:
            self.parse_error("Error while parsing wq-name: expected IDENT token")
    ...
    def parse_id_selector(self) -> IDSelector:
        ...
        assert hash_tok.val
        return IDSelector(hash_tok.val.val)

    def parse_class_selector(self) -> ClassSelector:
        self.expect(type=Tok.DELIM, val=".")
        ident = self.stream.accept(type=Tok.IDENT)
        if not ident:
            self.parse_error(
    ...
    def _parse_attribute_selector(self):
        ...
        self.stream.consume_whitespace()
        val = ""
        if self.stream.peek().type == Tok.STRING:
            val = self.stream.consume().val
        elif self.stream.peek().type == Tok.IDENT:
            val = self.stream.consume().val
        else:
            self.parse_error(
                f"Error while parsing attribute selector: expected a rhs value but got {self.stream.peek()}"
            )
        self.stream.consume_whitespace()

        modifier = ""
        if not self.stream.peek().type == Tok.EOF:
            modifier = self.parse_attr_modifier()

        return AttributeSelector(
            attr.casefold(), matcher, val=val, modifier=modifier, namespace=namespace
        )
    ...
    def is_subclass_selector(self) -> bool:
        ...
        if tok.type == Tok.HASH or tok.type == Tok.LBRAC:
            return True
        if tok.type == Tok.COLON:
            return self.stream.peek(2).type != Tok.COLON  # is a pseudo element ::
        if tok.type == Tok.DELIM and tok.val == ".":
            return True
        return False
