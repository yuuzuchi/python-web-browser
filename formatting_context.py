...
class BlockFormattingContext:
    ...
    def format(self):
        ...
        self.block.y = (
            self.block.previous.y + self.block.previous.height
            if self.block.previous
            else self.block.parent.y
        )
...
class InlineFormattingContext:
            ...
            if isinstance(layout, BreakLayout):
                ...
                for i, fragment in enumerate(fragments):
                    ...
                    self.add_fragment_to_line(
                        fragment, font, color, layout, pre=False, end=end
                    )
    ...
    def add_fragment_to_line(
        ...
        if not pre and self.cx + w > self.available_width:
            if "\u00ad" not in fragment:
                ...
                self.curr_line.children.append(
                    TextFragment(
                        layout_object,
                        lefthalf,
                        self.x + self.cx,
                ...
                self.flush_line(layout_object.node)
                self.add_fragment_to_line(
                    righthalf, font, color, layout_object, pre=pre
                )
                return

        frag = TextFragment(
            layout_object,
            fragment_without_hyphen,
            self.x + self.cx,
            self.y + self.cy,
            w,
            font,
    ...
    def flush_line(self, DOM_node_reference):
        ...
        if not self.curr_line.children:
            ...
            max_ascent = max(
                [
                    fragment.font.cached_metrics["ascent"]
                    for fragment in self.curr_line.children
                ]
            )
            max_descent = max(
                [
                    fragment.font.cached_metrics["descent"]
                    for fragment in self.curr_line.children
                ]
            )
