...
class DescendantSelector(Selector):
    ...
    def add_right(self, selector: Selector):
        ...
        self.specificity = tuple(
            a + b for a, b in zip(self.specificity, selector.specificity)
        )
...
class HasSelector(Selector):
    def __init__(self, target: Selector | None, selector_list: list[Selector]):
        parens_specificity = (
            max(selector.specificity for selector in selector_list)
            if selector_list
            else (0, 0, 0)
        )
    ...
    def set_selectors(self, selector_list: list[Selector]):
        ...
        self.specificity = tuple(
            a + b for a, b in zip(parens_specificity, target_specificity)
        )
            ...
            try:
                ...
                if prop == "font":
                    ...
                    for prop, val in pairs.items():
                        decl = Declaration(
                            prop, val, important, origin_priority, self.decl_count
                        )
                        if specificity:
                            decl.set_specificity(specificity)
            ...
            if "." in text:
                if text.startswith("."):
                    ...
                    return SelectorSequence(
                        TagSelector(parts[0]), ClassSelector("." + parts[1])
                    )
