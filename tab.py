...
class Tab:
    ...
    def draw(self):
        ...
        for cmd in self.display_list:
            if cmd.rect.top > self.scroll.pos + self.tab_height:
                continue
            if cmd.rect.bottom + MARGINS[3] < self.scroll.pos:
                continue
    ...
    def _layout(self):
        ...
        self.display_list = []
        paint_tree(self.document_layout, self.display_list)
        # print_layout_tree(self.document)
        # print_paint(self.display_list)
        elapsed_time = time.perf_counter() - start_time
        print(
            f"layout() {self.canvas.winfo_width()}x{self.canvas.winfo_height()} in{elapsed_time: .6f} seconds, {len(self.display_list)} nodes"
        )
        self.text_height = max(self.document_layout.height, 0)
        self.invalidate()
    ...
    def navigate(self, url_str: str, from_user_input: bool = False):
        ...
        self.history_manager.append(str(url_obj))
    ...
    def go_back(self):
        if self.can_go_back():
            ...
            self.load(
                back
            )  # TODO: call resolve on back nav, so we get url.fragment_no_render_required
    ...
    def jump_to_fragment(self, target_fragment: str, scroll_animation=True):
        ...
        for fragment in tree_to_fragment_list(self.document_layout):
    ...
    def update_scroll(self):
        ...
        if (
            abs(self.scroll.velocity) < 0.1
            and abs(self.scroll.target_pos - self.scroll.pos) < 0.5
        ):
        ...
        self.scroll.target_pos = min(
            self.scroll.target_pos, self.text_height - height + MARGINS[3]
        )
    ...
    def on_leftmouse_down(self, x, y):
        ...
        if (
            y >= self.scroll.bar_y
            and y <= self.scroll.bar_y + self.scroll.bar_height
            and x >= width - self.scroll.bar_width
            and x < width
        ):
    ...
    def handle_drag_scroll(self, x, y):
        ...
        # absolute scroll (click-to-jump)
            ...
            self.scroll.target_pos = self.scroll.pos = (
                self.text_height - self.tab_height
            ) * screen_percent
            self.invalidate()
